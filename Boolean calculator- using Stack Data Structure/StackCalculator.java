...
public class StackCalculator extends Calculator{
	...
	public String infixToPostfix(String expr) {
		...
		return post_expr.substring(0, post_expr.length()-1);}//return the postfix expr without space in the end.
		...
		while (input_expr.hasMoreElements()) {
			...
			if (postoken instanceof Op) {
				...
				if (postoken instanceof BinaryOp) {	
					...
					result = (boolean)((BinaryOp)postoken).operate(((ValueToken) first).getValue(), ((ValueToken) second).getValue());//we turn first and second to boolean so we can use operate
